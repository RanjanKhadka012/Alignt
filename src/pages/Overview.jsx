...
export default function Overview() {
  ...
  const { initiativeResults, overallReadiness, criticalTalent, documentationCoverage } = useDerivedData()
  const ranked = [...initiativeResults].sort((a, b) => a.readinessPct - b.readinessPct)
  ...
  return <div className="readiness-page">
    <header className="readiness-heading"><span className="readiness-data">STRATEGY INTELLIGENCE</span><h1>Can we execute our strategy?</h1><p>Connect workforce capability to the outcomes your business is counting on.</p></header>
    {!readinessDataComplete && <p className="readiness-data-notice">Preview with existing demo records. The requested 150-employee dataset and four role profiles have not been supplied; readiness below does not represent that workforce.</p>}
    <section className="readiness-panel readiness-hero"><ReadinessRing value={overallReadiness}/><div><span className="readiness-data">EXECUTION READINESS</span><h2>Your workforce is {overallReadiness}% ready to execute your stated strategy.</h2>{worst && <p>{worst.label} has the lowest readiness at <span className="readiness-data">{worst.readinessPct}%</span>. {worst.relevantWorkforce ? `${worst.gap} of ${worst.relevantWorkforce} relevant employees have no qualifying skill recorded, putting delivery capacity at risk.` : 'No employees in its relevant roles are recorded, so capability cannot yet be confirmed.'}</p>}<small>Weighted by relevant workforce. A qualifying skill record counts as coverage, regardless of proficiency; employees may contribute to multiple objectives.</small></div></section>
    <section className="readiness-panel readiness-objectives"><header><h2>Readiness by objective</h2><span className="readiness-data">{initiativeResults.length} OBJECTIVES</span></header>{initiativeResults.map(initiative => <InitiativeRow key={initiative.id} initiative={initiative}/>)}{!initiativeResults.length && <p>No objectives configured yet.</p>}</section>
    <ConsequenceCallout initiative={risk}/>
