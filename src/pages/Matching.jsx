...
export default function Matching() {
  ...
  const { initiativeResults } = useDerivedData()
  const sourceGoal = initiativeResults.find(item => item.id === params.get('goal'))
  ...
  useEffect(() => {
    if (sourceGoal) { setGoal(sourceGoal.label); setTimeline(sourceGoal.targetDate ? `By ${sourceGoal.targetDate}` : 'No target date specified'); setResult(null); setError('') }
  }, [sourceGoal?.id, sourceGoal?.label, sourceGoal?.targetDate])
  ...
  return <div className="matching-page">
    ...
    {sourceGoal && <p className="matching-muted">Continuing from Overview: {sourceGoal.label}. Analyze this goal to validate capability coverage and compare planning scenarios. <Link to={`/recommendations?goal=${encodeURIComponent(sourceGoal.id)}`}>Review employee development needs ↗</Link></p>}
    ...
    {result && <div className="goal-risk-results" key={result.id}>
      ...
      {!analysis.pool.length ? <p>No relevant workforce pool; staffing costs cannot be meaningfully estimated.</p> : <><FinancialBarChart optionA={finances.optionA} recommended={finances.recommended.total}/><div className="financial-grid"><FinancialComparisonCard title="Recruit for every skill-gapped position" total={finances.optionA}><p>{analysis.gapped.length} hires × {formatMoney(assumptions.avgSpecialistSalary * (1 + assumptions.recruitingOverheadPct))}</p><small>First-year salary + recruiting overhead. Does not include training the trainable group.</small></FinancialComparisonCard><FinancialComparisonCard recommended title="Develop internally. Recruit selectively." total={finances.recommended.total}><ul><li>{analysis.trainable.length} trainable × {formatMoney(assumptions.trainableCoursePerPerson)} · ~6 weeks</li><li>{finances.recommended.upskillGappedCount} skill-gapped × {formatMoney(assumptions.gappedCoursePerPerson)} · ~10 weeks</li><li>{finances.recommended.recruitCount} illustrative safety-margin recruits × {formatMoney(assumptions.avgSpecialistSalary * (1 + assumptions.recruitingOverheadPct))}</li></ul><strong className="financial-savings">{formatMoney(Math.abs(finances.recommended.savingsVsOptionA))} {finances.recommended.savingsVsOptionA >= 0 ? 'savings' : 'additional investment'} vs. Option A</strong></FinancialComparisonCard></div><p className="matching-muted matching-footnote">Scenarios cover different populations: blended includes training the trainable group. Courses are assumptions, not proof of qualification. The placeholder recruit count is a cost scenario, not a hiring recommendation. Validate training feasibility and internal capacity before setting a hiring target. Safety-margin hires require appropriate skills; these counts do not guarantee every capability gap is closed. Training excludes paid learning time and backfill.</p></>}
      </section>
