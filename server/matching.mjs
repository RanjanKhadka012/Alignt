...
export function matchingHandler(env = process.env) {
  return async (req, res, next) => {
    ...
    if (!env.OPENAI_API_KEY && !env.GEMINI_API_KEY) return send(503, { error: 'AI matching is not configured. Ask your administrator to configure the matching service.' })
    try {
      let body = ''
      ...
      send(502, { error: error.name === 'TimeoutError' ? 'The AI service timed out. Please try again.' : (error.message?.startsWith('OpenAI ') || error.message?.startsWith('The AI service ')) ? error.message : 'Could not reach the AI service. Please try again.' })
    }
  }
}
