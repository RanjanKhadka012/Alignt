...
import { openrouterResponse } from './openrouter.mjs'

export async function aiResponse(env, body) {
  const preferred = (env.PREFERRED_AI_PROVIDER || '').toLowerCase()
  if (preferred === 'openrouter' && env.OPENROUTER_API_KEY) return openrouterResponse(env, body)
  ...
  if (env.OPENROUTER_API_KEY) return openrouterResponse(env, body)
  ...
  throw new Error('No AI provider configured. Set OPENROUTER_API_KEY, OPENAI_API_KEY, ALLAMA_API_KEY or GEMINI_API_KEY on the server.')
}
