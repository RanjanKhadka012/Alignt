...
import { findAiEmployee } from './aiWorkforceData.mjs'
...
export function recommendationsHandler(env = process.env) {
  ...
  return async (req, res, next) => {
    ...
    if (!env.OPENROUTER_API_KEY && !env.OPENAI_API_KEY && !env.GEMINI_API_KEY && !env.ALLAMA_API_KEY) return send(503, { error: 'Configure an AI provider (OpenRouter, OpenAI, Allama, or Gemini) and model to enable live role reviews.' })
    try {
      let raw = ''
